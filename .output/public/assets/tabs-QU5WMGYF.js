import { r as e, t } from "./jsx-runtime-BkSabwWG.js";
import { c as n } from "./useStore-D1ICS8_H.js";
import { a as r, n as i } from "./button-Fem7RhN8.js";
import {
  _ as a,
  b as o,
  f as s,
  g as c,
  v as l,
  w as u,
  x as d,
  y as f,
} from "./index-C957XaZb.js";
import { n as p, t as m } from "./dist-Cgt23J8B.js";
var h = e(n(), 1),
  g = Object.defineProperty,
  _ = (e, t) => g(e, `name`, { value: t, configurable: !0 }),
  v = !1;
function y() {
  let [e, t] = h.useState(v);
  return (
    h.useEffect(() => {
      v || ((v = !0), t(!0));
    }, []),
    e
  );
}
_(y, `useIsHydrated`);
var b = h.useSyncExternalStore;
function x() {
  return () => {};
}
_(x, `subscribe`);
function S() {
  return b(
    x,
    () => !0,
    () => !1,
  );
}
_(S, `useIsHydratedModern`);
var C = typeof b == `function` ? S : y,
  w = t(),
  T = Object.defineProperty,
  E = (e, t) => T(e, `name`, { value: t, configurable: !0 }),
  D = `rovingFocusGroup.onEntryFocus`,
  ee = { bubbles: !1, cancelable: !0 },
  O = `RovingFocusGroup`,
  [k, A, j] = p(O),
  [M, N] = o(O, [j]),
  [te, P] = M(O),
  ne = h.forwardRef(
    E(function (e, t) {
      return (0, w.jsx)(k.Provider, {
        scope: e.__scopeRovingFocusGroup,
        children: (0, w.jsx)(k.Slot, {
          scope: e.__scopeRovingFocusGroup,
          children: (0, w.jsx)(re, { ...e, ref: t }),
        }),
      });
    }, `RovingFocusGroup`),
  ),
  re = h.forwardRef(
    E(function (e, t) {
      let {
          __scopeRovingFocusGroup: n,
          orientation: i,
          loop: o = !1,
          dir: s,
          currentTabStopId: l,
          defaultCurrentTabStopId: f,
          onCurrentTabStopIdChange: p,
          onEntryFocus: g,
          preventScrollOnEntryFocus: _ = !1,
          ...v
        } = e,
        y = h.useRef(null),
        b = r(t, y),
        x = m(s),
        [S, C] = a({ prop: l, defaultProp: f ?? null, onChange: p, caller: O }),
        [T, E] = h.useState(!1),
        k = c(g),
        j = A(n),
        M = h.useRef(!1),
        [N, P] = h.useState(0);
      return (
        h.useEffect(() => {
          let e = y.current;
          if (e)
            return (
              e.addEventListener(D, k),
              () => e.removeEventListener(D, k)
            );
        }, [k]),
        (0, w.jsx)(te, {
          scope: n,
          orientation: i,
          dir: x,
          loop: o,
          currentTabStopId: S,
          onItemFocus: h.useCallback((e) => C(e), [C]),
          onItemShiftTab: h.useCallback(() => E(!0), []),
          onFocusableItemAdd: h.useCallback(() => P((e) => e + 1), []),
          onFocusableItemRemove: h.useCallback(() => P((e) => e - 1), []),
          children: (0, w.jsx)(u.div, {
            tabIndex: T || N === 0 ? -1 : 0,
            "data-orientation": i,
            ...v,
            ref: b,
            style: { outline: `none`, ...e.style },
            onMouseDown: d(e.onMouseDown, () => {
              M.current = !0;
            }),
            onFocus: d(e.onFocus, (e) => {
              let t = !M.current;
              if (e.target === e.currentTarget && t && !T) {
                let t = new CustomEvent(D, ee);
                if ((e.currentTarget.dispatchEvent(t), !t.defaultPrevented)) {
                  let e = j().filter((e) => e.focusable);
                  L(
                    [e.find((e) => e.active), e.find((e) => e.id === S), ...e]
                      .filter(Boolean)
                      .map((e) => e.ref.current),
                    _,
                  );
                }
              }
              M.current = !1;
            }),
            onBlur: d(e.onBlur, () => E(!1)),
          }),
        })
      );
    }, `RovingFocusGroupImpl`),
  ),
  ie = `RovingFocusGroupItem`,
  ae = h.forwardRef(
    E(function (e, t) {
      let {
          __scopeRovingFocusGroup: n,
          focusable: r = !0,
          active: i = !1,
          tabStopId: a,
          children: o,
          ...s
        } = e,
        c = l(),
        p = a || c,
        m = P(ie, n),
        g = m.currentTabStopId === p,
        _ = A(n),
        {
          onFocusableItemAdd: v,
          onFocusableItemRemove: y,
          currentTabStopId: b,
        } = m,
        x = C();
      return (
        f(() => {
          if (!(!x || !r)) return (v(), () => y());
        }, [x, r, v, y]),
        h.useEffect(() => {
          if (!(x || !r)) return (v(), () => y());
        }, [x, r, v, y]),
        (0, w.jsx)(k.ItemSlot, {
          scope: n,
          id: p,
          focusable: r,
          active: i,
          children: (0, w.jsx)(u.span, {
            tabIndex: g ? 0 : -1,
            "data-orientation": m.orientation,
            ...s,
            ref: t,
            onMouseDown: d(e.onMouseDown, (e) => {
              r ? m.onItemFocus(p) : e.preventDefault();
            }),
            onFocus: d(e.onFocus, () => m.onItemFocus(p)),
            onKeyDown: d(e.onKeyDown, (e) => {
              if (e.key === `Tab` && e.shiftKey) {
                m.onItemShiftTab();
                return;
              }
              if (e.target !== e.currentTarget) return;
              let t = I(e, m.orientation, m.dir);
              if (t !== void 0) {
                if (e.metaKey || e.ctrlKey || e.altKey || e.shiftKey) return;
                e.preventDefault();
                let n = _()
                  .filter((e) => e.focusable)
                  .map((e) => e.ref.current);
                if (t === `last`) n.reverse();
                else if (t === `prev` || t === `next`) {
                  t === `prev` && n.reverse();
                  let r = n.indexOf(e.currentTarget);
                  n = m.loop ? R(n, r + 1) : n.slice(r + 1);
                }
                setTimeout(() => L(n));
              }
            }),
            children:
              typeof o == `function`
                ? o({ isCurrentTabStop: g, hasTabStop: b != null })
                : o,
          }),
        })
      );
    }, `RovingFocusGroupItem`),
  ),
  oe = {
    ArrowLeft: `prev`,
    ArrowUp: `prev`,
    ArrowRight: `next`,
    ArrowDown: `next`,
    PageUp: `first`,
    Home: `first`,
    PageDown: `last`,
    End: `last`,
  };
function F(e, t) {
  return t === `rtl`
    ? e === `ArrowLeft`
      ? `ArrowRight`
      : e === `ArrowRight`
        ? `ArrowLeft`
        : e
    : e;
}
E(F, `getDirectionAwareKey`);
function I(e, t, n) {
  let r = F(e.key, n);
  if (
    !(t === `vertical` && [`ArrowLeft`, `ArrowRight`].includes(r)) &&
    !(t === `horizontal` && [`ArrowUp`, `ArrowDown`].includes(r))
  )
    return oe[r];
}
E(I, `getFocusIntent`);
function L(e, t = !1) {
  let n = document.activeElement;
  for (let r of e)
    if (
      r === n ||
      (r.focus({ preventScroll: t }), document.activeElement !== n)
    )
      return;
}
E(L, `focusFirst`);
function R(e, t) {
  return e.map((n, r) => e[(t + r) % e.length]);
}
E(R, `wrapArray`);
var z = ne,
  B = ae,
  se = Object.defineProperty,
  V = (e, t) => se(e, `name`, { value: t, configurable: !0 }),
  H = `Tabs`,
  [ce, le] = o(H, [N]),
  U = N(),
  [ue, W] = ce(H),
  G = h.forwardRef(
    V(function (e, t) {
      let {
          __scopeTabs: n,
          value: r,
          onValueChange: i,
          defaultValue: o,
          orientation: s = `horizontal`,
          dir: c,
          activationMode: d = `automatic`,
          ...f
        } = e,
        p = m(c),
        [h, g] = a({ prop: r, onChange: i, defaultProp: o ?? ``, caller: H });
      return (0, w.jsx)(ue, {
        scope: n,
        baseId: l(),
        value: h,
        onValueChange: g,
        orientation: s,
        dir: p,
        activationMode: d,
        children: (0, w.jsx)(u.div, {
          dir: p,
          "data-orientation": s,
          ...f,
          ref: t,
        }),
      });
    }, `Tabs`),
  ),
  de = `TabsList`,
  fe = h.forwardRef(
    V(function (e, t) {
      let { __scopeTabs: n, loop: r = !0, ...i } = e,
        a = W(de, n),
        o = U(n);
      return (0, w.jsx)(z, {
        asChild: !0,
        ...o,
        orientation: a.orientation,
        dir: a.dir,
        loop: r,
        children: (0, w.jsx)(u.div, {
          role: `tablist`,
          "aria-orientation": a.orientation,
          ...i,
          ref: t,
        }),
      });
    }, `TabsList`),
  ),
  pe = `TabsTrigger`,
  me = h.forwardRef(
    V(function (e, t) {
      let { __scopeTabs: n, value: r, disabled: i = !1, ...a } = e,
        o = W(pe, n),
        s = U(n),
        c = K(o.baseId, r),
        l = q(o.baseId, r),
        f = r === o.value;
      return (0, w.jsx)(B, {
        asChild: !0,
        ...s,
        focusable: !i,
        active: f,
        children: (0, w.jsx)(u.button, {
          type: `button`,
          role: `tab`,
          "aria-selected": f,
          "aria-controls": l,
          "data-state": f ? `active` : `inactive`,
          "data-disabled": i ? `` : void 0,
          disabled: i,
          id: c,
          ...a,
          ref: t,
          onMouseDown: d(e.onMouseDown, (e) => {
            !i && e.button === 0 && e.ctrlKey === !1
              ? o.onValueChange(r)
              : e.preventDefault();
          }),
          onKeyDown: d(e.onKeyDown, (e) => {
            i ||
              e.target !== e.currentTarget ||
              ([` `, `Enter`].includes(e.key) && o.onValueChange(r));
          }),
          onFocus: d(e.onFocus, () => {
            let e = o.activationMode !== `manual`;
            !f && !i && e && o.onValueChange(r);
          }),
        }),
      });
    }, `TabsTrigger`),
  ),
  he = `TabsContent`,
  ge = h.forwardRef(
    V(function (e, t) {
      let { __scopeTabs: n, value: r, forceMount: i, children: a, ...o } = e,
        c = W(he, n),
        l = K(c.baseId, r),
        d = q(c.baseId, r),
        f = r === c.value,
        p = h.useRef(f);
      return (
        h.useEffect(() => {
          let e = requestAnimationFrame(() => (p.current = !1));
          return () => cancelAnimationFrame(e);
        }, []),
        (0, w.jsx)(s, {
          present: i || f,
          children: ({ present: n }) =>
            (0, w.jsx)(u.div, {
              "data-state": f ? `active` : `inactive`,
              "data-orientation": c.orientation,
              role: `tabpanel`,
              "aria-labelledby": l,
              hidden: !n,
              id: d,
              tabIndex: 0,
              ...o,
              ref: t,
              style: {
                ...e.style,
                animationDuration: p.current ? `0s` : void 0,
              },
              children: n && a,
            }),
        })
      );
    }, `TabsContent`),
  );
function K(e, t) {
  return `${e}-trigger-${t}`;
}
V(K, `makeTriggerId`);
function q(e, t) {
  return `${e}-content-${t}`;
}
V(q, `makeContentId`);
var _e = G,
  J = fe,
  Y = me,
  X = ge,
  ve = _e,
  Z = h.forwardRef(({ className: e, ...t }, n) =>
    (0, w.jsx)(J, {
      ref: n,
      className: i(
        `inline-flex h-9 items-center justify-center rounded-lg bg-muted p-1 text-muted-foreground`,
        e,
      ),
      ...t,
    }),
  );
Z.displayName = J.displayName;
var Q = h.forwardRef(({ className: e, ...t }, n) =>
  (0, w.jsx)(Y, {
    ref: n,
    className: i(
      `inline-flex items-center justify-center whitespace-nowrap rounded-md px-3 py-1 text-sm font-medium ring-offset-background cursor-pointer transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 disabled:cursor-not-allowed data-[state=active]:bg-background data-[state=active]:text-foreground data-[state=active]:shadow`,
      e,
    ),
    ...t,
  }),
);
Q.displayName = Y.displayName;
var $ = h.forwardRef(({ className: e, ...t }, n) =>
  (0, w.jsx)(X, {
    ref: n,
    className: i(
      `mt-2 ring-offset-background focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2`,
      e,
    ),
    ...t,
  }),
);
$.displayName = X.displayName;
export { B as a, Q as i, $ as n, z as o, Z as r, N as s, ve as t };
