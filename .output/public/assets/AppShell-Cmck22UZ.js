import { r as e, t } from "./jsx-runtime-BkSabwWG.js";
import { c as n, i as r, t as i } from "./useStore-D1ICS8_H.js";
import { a, i as o, n as s, r as c, s as l, t as u } from "./Logo-BzB7YJf1.js";
import { c as d, n as f, o as p } from "./button-Fem7RhN8.js";
function m(e) {
  let t = r({ warn: e?.router === void 0 }),
    n = e?.router || t;
  return i(n.stores.__store, d(e, n));
}
var h = a(`bell`, [
    [`path`, { d: `M10.268 21a2 2 0 0 0 3.464 0`, key: `vwvbt9` }],
    [
      `path`,
      {
        d: `M3.262 15.326A1 1 0 0 0 4 17h16a1 1 0 0 0 .74-1.673C19.41 13.956 18 12.499 18 8A6 6 0 0 0 6 8c0 4.499-1.411 5.956-2.738 7.326`,
        key: `11g9vi`,
      },
    ],
  ]),
  g = a(`building-2`, [
    [`path`, { d: `M10 12h4`, key: `a56b0p` }],
    [`path`, { d: `M10 8h4`, key: `1sr2af` }],
    [`path`, { d: `M14 21v-3a2 2 0 0 0-4 0v3`, key: `1rgiei` }],
    [
      `path`,
      {
        d: `M6 10H4a2 2 0 0 0-2 2v7a2 2 0 0 0 2 2h16a2 2 0 0 0 2-2V9a2 2 0 0 0-2-2h-2`,
        key: `secmi2`,
      },
    ],
    [`path`, { d: `M6 21V5a2 2 0 0 1 2-2h8a2 2 0 0 1 2 2v16`, key: `16ra0t` }],
  ]),
  _ = a(`calendar-days`, [
    [`path`, { d: `M8 2v4`, key: `1cmpym` }],
    [`path`, { d: `M16 2v4`, key: `4m81vk` }],
    [
      `rect`,
      { width: `18`, height: `18`, x: `3`, y: `4`, rx: `2`, key: `1hopcy` },
    ],
    [`path`, { d: `M3 10h18`, key: `8toen8` }],
    [`path`, { d: `M8 14h.01`, key: `6423bh` }],
    [`path`, { d: `M12 14h.01`, key: `1etili` }],
    [`path`, { d: `M16 14h.01`, key: `1gbofw` }],
    [`path`, { d: `M8 18h.01`, key: `lrp35t` }],
    [`path`, { d: `M12 18h.01`, key: `mhygvu` }],
    [`path`, { d: `M16 18h.01`, key: `kzsmim` }],
  ]),
  v = a(`chevron-down`, [[`path`, { d: `m6 9 6 6 6-6`, key: `qrunsl` }]]),
  y = a(`file-text`, [
    [
      `path`,
      {
        d: `M6 22a2 2 0 0 1-2-2V4a2 2 0 0 1 2-2h8a2.4 2.4 0 0 1 1.704.706l3.588 3.588A2.4 2.4 0 0 1 20 8v12a2 2 0 0 1-2 2z`,
        key: `1oefj6`,
      },
    ],
    [`path`, { d: `M14 2v5a1 1 0 0 0 1 1h5`, key: `wfsgrz` }],
    [`path`, { d: `M10 9H8`, key: `b1mrlr` }],
    [`path`, { d: `M16 13H8`, key: `t4e002` }],
    [`path`, { d: `M16 17H8`, key: `z1uh3a` }],
  ]),
  b = a(`house`, [
    [
      `path`,
      { d: `M15 21v-8a1 1 0 0 0-1-1h-4a1 1 0 0 0-1 1v8`, key: `5wwlr5` },
    ],
    [
      `path`,
      {
        d: `M3 10a2 2 0 0 1 .709-1.528l7-6a2 2 0 0 1 2.582 0l7 6A2 2 0 0 1 21 10v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z`,
        key: `r6nss1`,
      },
    ],
  ]),
  x = a(`linkedin`, [
    [
      `path`,
      {
        d: `M16 8a6 6 0 0 1 6 6v7h-4v-7a2 2 0 0 0-2-2 2 2 0 0 0-2 2v7h-4v-7a6 6 0 0 1 6-6z`,
        key: `c2jq9f`,
      },
    ],
    [`rect`, { width: `4`, height: `12`, x: `2`, y: `9`, key: `mk3on5` }],
    [`circle`, { cx: `4`, cy: `4`, r: `2`, key: `bt5ra8` }],
  ]),
  S = a(`list-checks`, [
    [`path`, { d: `M13 5h8`, key: `a7qcls` }],
    [`path`, { d: `M13 12h8`, key: `h98zly` }],
    [`path`, { d: `M13 19h8`, key: `c3s6r1` }],
    [`path`, { d: `m3 17 2 2 4-4`, key: `1jhpwq` }],
    [`path`, { d: `m3 7 2 2 4-4`, key: `1obspn` }],
  ]),
  C = a(`menu`, [
    [`path`, { d: `M4 5h16`, key: `1tepv9` }],
    [`path`, { d: `M4 12h16`, key: `1lakjw` }],
    [`path`, { d: `M4 19h16`, key: `1djgab` }],
  ]),
  w = a(`message-square`, [
    [
      `path`,
      {
        d: `M22 17a2 2 0 0 1-2 2H6.828a2 2 0 0 0-1.414.586l-2.202 2.202A.71.71 0 0 1 2 21.286V5a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2z`,
        key: `18887p`,
      },
    ],
  ]),
  T = a(`plug`, [
    [`path`, { d: `M12 22v-5`, key: `1ega77` }],
    [`path`, { d: `M15 8V2`, key: `18g5xt` }],
    [
      `path`,
      {
        d: `M17 8a1 1 0 0 1 1 1v4a4 4 0 0 1-4 4h-4a4 4 0 0 1-4-4V9a1 1 0 0 1 1-1z`,
        key: `1xoxul`,
      },
    ],
    [`path`, { d: `M9 8V2`, key: `14iosj` }],
  ]),
  E = a(`plus`, [
    [`path`, { d: `M5 12h14`, key: `1ays0h` }],
    [`path`, { d: `M12 5v14`, key: `s699le` }],
  ]),
  D = a(`scan-line`, [
    [`path`, { d: `M3 7V5a2 2 0 0 1 2-2h2`, key: `aa7l1z` }],
    [`path`, { d: `M17 3h2a2 2 0 0 1 2 2v2`, key: `4qcy5o` }],
    [`path`, { d: `M21 17v2a2 2 0 0 1-2 2h-2`, key: `6vwrx8` }],
    [`path`, { d: `M7 21H5a2 2 0 0 1-2-2v-2`, key: `ioqczr` }],
    [`path`, { d: `M7 12h10`, key: `b7w52i` }],
  ]),
  O = a(`search`, [
    [`path`, { d: `m21 21-4.34-4.34`, key: `14j7rj` }],
    [`circle`, { cx: `11`, cy: `11`, r: `8`, key: `4ej97u` }],
  ]),
  k = a(`settings`, [
    [
      `path`,
      {
        d: `M9.671 4.136a2.34 2.34 0 0 1 4.659 0 2.34 2.34 0 0 0 3.319 1.915 2.34 2.34 0 0 1 2.33 4.033 2.34 2.34 0 0 0 0 3.831 2.34 2.34 0 0 1-2.33 4.033 2.34 2.34 0 0 0-3.319 1.915 2.34 2.34 0 0 1-4.659 0 2.34 2.34 0 0 0-3.32-1.915 2.34 2.34 0 0 1-2.33-4.033 2.34 2.34 0 0 0 0-3.831A2.34 2.34 0 0 1 6.35 6.051a2.34 2.34 0 0 0 3.319-1.915`,
        key: `1i5ecw`,
      },
    ],
    [`circle`, { cx: `12`, cy: `12`, r: `3`, key: `1v7zrd` }],
  ]),
  A = a(`target`, [
    [`circle`, { cx: `12`, cy: `12`, r: `10`, key: `1mglay` }],
    [`circle`, { cx: `12`, cy: `12`, r: `6`, key: `1vlfrh` }],
    [`circle`, { cx: `12`, cy: `12`, r: `2`, key: `1c9p78` }],
  ]),
  j = a(`upload`, [
    [`path`, { d: `M12 3v12`, key: `1x0j5s` }],
    [`path`, { d: `m17 8-5-5-5 5`, key: `7q97r8` }],
    [`path`, { d: `M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4`, key: `ih7n3h` }],
  ]),
  M = a(`user-round`, [
    [`circle`, { cx: `12`, cy: `8`, r: `5`, key: `1hypcn` }],
    [`path`, { d: `M20 21a8 8 0 0 0-16 0`, key: `rfgkzh` }],
  ]),
  N = a(`users`, [
    [`path`, { d: `M16 21v-2a4 4 0 0 0-4-4H6a4 4 0 0 0-4 4v2`, key: `1yyitq` }],
    [`path`, { d: `M16 3.128a4 4 0 0 1 0 7.744`, key: `16gr8j` }],
    [`path`, { d: `M22 21v-2a4 4 0 0 0-3-3.87`, key: `kshegd` }],
    [`circle`, { cx: `9`, cy: `7`, r: `4`, key: `nufk8` }],
  ]),
  P = a(`wand-sparkles`, [
    [
      `path`,
      {
        d: `m21.64 3.64-1.28-1.28a1.21 1.21 0 0 0-1.72 0L2.36 18.64a1.21 1.21 0 0 0 0 1.72l1.28 1.28a1.2 1.2 0 0 0 1.72 0L21.64 5.36a1.2 1.2 0 0 0 0-1.72`,
        key: `ul74o6`,
      },
    ],
    [`path`, { d: `m14 7 3 3`, key: `1r5n42` }],
    [`path`, { d: `M5 6v4`, key: `ilb8ba` }],
    [`path`, { d: `M19 14v4`, key: `blhpug` }],
    [`path`, { d: `M10 2v2`, key: `7u0qdc` }],
    [`path`, { d: `M7 8H3`, key: `zfb6yr` }],
    [`path`, { d: `M21 16h-4`, key: `1cnmox` }],
    [`path`, { d: `M11 3H9`, key: `1obp7u` }],
  ]),
  F = e(n(), 1),
  I = t(),
  L = [
    { label: `Accueil`, icon: b, to: `/` },
    { label: `Candidatures`, icon: S, to: `/candidatures` },
    { label: `Opportunités`, icon: A, to: `/opportunites` },
    { label: `Entreprises`, icon: g, to: `/entreprises` },
    { label: `Contacts`, icon: N, to: `/contacts` },
    { label: `Calendrier`, icon: _, to: `/calendrier` },
    { label: `Documents`, icon: y, to: `/documents` },
    { label: `Importer`, icon: j, to: `/import` },
  ],
  R = [
    { label: `Match IA`, icon: c, to: `/assistant/match`, sub: !0 },
    { label: `CV Optimizer`, icon: D, to: `/profil`, sub: !0 },
    { label: `Email Assistant`, icon: o, to: `/contacts`, sub: !0 },
    {
      label: `LinkedIn Assistant`,
      icon: x,
      to: `/assistant/linkedin`,
      sub: !0,
    },
    { label: `Interview Coach`, icon: w, to: `/assistant/interview`, sub: !0 },
    { label: `Connecter une IA`, icon: T, to: `/assistant/connect`, sub: !0 },
  ],
  z = { label: `NACORA AI (Hub)`, icon: P, to: `/assistant` },
  B = () => l(`Bientôt disponible dans NACORA.`);
function V({ item: e, active: t }) {
  let n = (0, I.jsxs)(I.Fragment, {
      children: [
        (0, I.jsx)(e.icon, { className: `size-[18px] shrink-0` }),
        (0, I.jsx)(`span`, { className: `truncate`, children: e.label }),
      ],
    }),
    r = f(
      `group relative flex w-full items-center gap-3 rounded-xl py-2.5 text-[13.5px] font-medium transition-colors`,
      e.sub ? `pl-4 pr-3 text-[13px]` : `px-3`,
      t
        ? `bg-primary/15 text-foreground ring-1 ring-primary/35`
        : `text-muted-foreground hover:bg-sidebar-accent/50 hover:text-foreground`,
    );
  return e.to
    ? (0, I.jsxs)(p, {
        to: e.to,
        className: r,
        children: [
          t &&
            (0, I.jsx)(`span`, {
              className: `absolute left-0 top-1/2 h-5 w-[3px] -translate-y-1/2 rounded-r-full bg-primary`,
            }),
          n,
        ],
      })
    : (0, I.jsx)(`button`, {
        type: `button`,
        onClick: B,
        className: r,
        children: n,
      });
}
function H({
  title: e,
  subtitle: t,
  eyebrow: n,
  headerExtra: r,
  actions: i,
  onAdd: a,
  onSearch: o,
  searchValue: c,
  children: l,
}) {
  let d = m({ select: (e) => e.location.pathname }),
    [g, x] = (0, F.useState)(``),
    S = c ?? g,
    [w, T] = (0, F.useState)(!1);
  return (
    (0, F.useEffect)(() => {
      T(!1);
    }, [d]),
    (0, F.useEffect)(() => {
      if (!w) return;
      let e = document.body.style.overflow;
      return (
        (document.body.style.overflow = `hidden`),
        () => {
          document.body.style.overflow = e;
        }
      );
    }, [w]),
    (0, I.jsxs)(`div`, {
      className: `min-h-screen bg-background aurora-bg`,
      children: [
        (0, I.jsxs)(`div`, {
          className: f(
            `fixed inset-0 z-50 md:hidden`,
            w ? `pointer-events-auto` : `pointer-events-none`,
          ),
          "aria-hidden": !w,
          children: [
            (0, I.jsx)(`button`, {
              type: `button`,
              "aria-label": `Fermer le menu`,
              onClick: () => T(!1),
              className: f(
                `absolute inset-0 bg-background/70 backdrop-blur-sm transition-opacity duration-300`,
                w ? `opacity-100` : `opacity-0`,
              ),
            }),
            (0, I.jsxs)(`aside`, {
              className: f(
                `absolute inset-y-0 left-0 flex w-[82%] max-w-[300px] flex-col border-r border-sidebar-border bg-sidebar/95 backdrop-blur-xl transition-transform duration-300 ease-out`,
                w ? `translate-x-0` : `-translate-x-full`,
              ),
              children: [
                (0, I.jsxs)(`div`, {
                  className: `flex h-[68px] shrink-0 items-center justify-between gap-2 px-4`,
                  children: [
                    (0, I.jsx)(u, {}),
                    (0, I.jsx)(`button`, {
                      type: `button`,
                      "aria-label": `Fermer le menu`,
                      onClick: () => T(!1),
                      className: `grid size-9 shrink-0 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-sidebar-accent/50 hover:text-foreground`,
                      children: (0, I.jsx)(s, { className: `size-5` }),
                    }),
                  ],
                }),
                (0, I.jsxs)(`nav`, {
                  className: `flex-1 overflow-y-auto px-3 pb-4`,
                  children: [
                    (0, I.jsx)(`div`, {
                      className: `flex flex-col gap-1`,
                      children: L.map((e) =>
                        (0, I.jsx)(V, { item: e, active: e.to === d }, e.label),
                      ),
                    }),
                    (0, I.jsx)(`div`, {
                      className: `my-4 h-px bg-sidebar-border`,
                    }),
                    (0, I.jsxs)(`p`, {
                      className: `mb-2 flex items-center gap-1.5 px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground`,
                      children: [
                        `NACORA AI `,
                        (0, I.jsx)(`span`, {
                          className: `text-primary`,
                          children: `✦`,
                        }),
                      ],
                    }),
                    (0, I.jsx)(V, { item: z, active: d === `/assistant` }),
                    (0, I.jsx)(`div`, {
                      className: `mt-1 ml-5 flex flex-col gap-1 border-l border-sidebar-border pl-2`,
                      children: R.map((e) =>
                        (0, I.jsx)(V, { item: e, active: e.to === d }, e.label),
                      ),
                    }),
                    (0, I.jsx)(`div`, {
                      className: `my-4 h-px bg-sidebar-border`,
                    }),
                    (0, I.jsxs)(`div`, {
                      className: `flex flex-col gap-1`,
                      children: [
                        (0, I.jsx)(V, {
                          item: {
                            label: `Paramètres`,
                            icon: k,
                            to: `/parametres`,
                          },
                          active: d === `/parametres`,
                        }),
                        (0, I.jsx)(V, {
                          item: { label: `Mon profil`, icon: M, to: `/profil` },
                          active: d === `/profil`,
                        }),
                      ],
                    }),
                  ],
                }),
              ],
            }),
          ],
        }),
        (0, I.jsxs)(`aside`, {
          className: `fixed inset-y-0 left-0 z-40 hidden w-[248px] flex-col border-r border-sidebar-border bg-sidebar/90 backdrop-blur-xl md:flex`,
          children: [
            (0, I.jsx)(`div`, {
              className: `flex h-[72px] items-center px-5`,
              children: (0, I.jsx)(u, {}),
            }),
            (0, I.jsxs)(`nav`, {
              className: `flex-1 overflow-y-auto px-3 pb-4`,
              children: [
                (0, I.jsx)(`div`, {
                  className: `flex flex-col gap-1`,
                  children: L.map((e) =>
                    (0, I.jsx)(V, { item: e, active: e.to === d }, e.label),
                  ),
                }),
                (0, I.jsx)(`div`, { className: `my-4 h-px bg-sidebar-border` }),
                (0, I.jsxs)(`p`, {
                  className: `mb-2 flex items-center gap-1.5 px-3 text-[11px] font-semibold uppercase tracking-[0.14em] text-muted-foreground`,
                  children: [
                    `NACORA AI `,
                    (0, I.jsx)(`span`, {
                      className: `text-primary`,
                      children: `✦`,
                    }),
                  ],
                }),
                (0, I.jsx)(V, { item: z, active: d === `/assistant` }),
                (0, I.jsx)(`div`, {
                  className: `mt-1 ml-5 flex flex-col gap-1 border-l border-sidebar-border pl-2`,
                  children: R.map((e) =>
                    (0, I.jsx)(V, { item: e, active: e.to === d }, e.label),
                  ),
                }),
              ],
            }),
            (0, I.jsxs)(`div`, {
              className: `px-3 pb-3`,
              children: [
                (0, I.jsxs)(p, {
                  to: `/parametres`,
                  className: `flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-[13.5px] font-medium text-muted-foreground transition-colors hover:bg-sidebar-accent/50 hover:text-foreground`,
                  children: [
                    (0, I.jsx)(k, { className: `size-[18px]` }),
                    ` Paramètres`,
                  ],
                }),
                (0, I.jsxs)(p, {
                  to: `/profil`,
                  className: `mt-2 flex items-center gap-3 rounded-2xl border border-sidebar-border bg-card/70 px-3 py-2.5 transition-colors hover:bg-card`,
                  children: [
                    (0, I.jsx)(`span`, {
                      className: `grid size-9 shrink-0 place-items-center rounded-full gradient-hero text-primary-foreground`,
                      children: (0, I.jsx)(M, { className: `size-4` }),
                    }),
                    (0, I.jsxs)(`span`, {
                      className: `min-w-0 flex-1 text-left`,
                      children: [
                        (0, I.jsx)(`span`, {
                          className: `block truncate text-[13px] font-semibold`,
                          children: `Mon compte`,
                        }),
                        (0, I.jsx)(`span`, {
                          className: `block truncate text-[11px] text-muted-foreground`,
                          children: `Voir mon profil`,
                        }),
                      ],
                    }),
                    (0, I.jsx)(v, {
                      className: `size-4 shrink-0 text-muted-foreground`,
                    }),
                  ],
                }),
              ],
            }),
          ],
        }),
        (0, I.jsxs)(`div`, {
          className: `md:pl-[248px]`,
          children: [
            (0, I.jsx)(`header`, {
              className: `sticky top-0 z-30 border-b border-border/50 bg-background/75 backdrop-blur-xl`,
              children: (0, I.jsxs)(`div`, {
                className: `mx-auto flex h-auto max-w-[1200px] flex-wrap items-center gap-3 px-4 py-3.5 sm:px-6 md:h-[72px] md:flex-nowrap md:py-0`,
                children: [
                  (0, I.jsxs)(`div`, {
                    className: `flex w-full items-center gap-2 md:hidden`,
                    children: [
                      (0, I.jsx)(`button`, {
                        type: `button`,
                        onClick: () => T(!0),
                        "aria-label": `Ouvrir le menu`,
                        className: `press grid size-10 shrink-0 place-items-center rounded-full border border-border/60 bg-card/60 text-foreground`,
                        children: (0, I.jsx)(C, { className: `size-5` }),
                      }),
                      (0, I.jsx)(u, {}),
                      (0, I.jsxs)(`button`, {
                        type: `button`,
                        onClick: B,
                        "aria-label": `Notifications`,
                        className: `relative ml-auto grid size-10 shrink-0 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground`,
                        children: [
                          (0, I.jsx)(h, { className: `size-[18px]` }),
                          (0, I.jsx)(`span`, {
                            className: `absolute right-1.5 top-1.5 grid size-4 place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground`,
                            children: `3`,
                          }),
                        ],
                      }),
                    ],
                  }),
                  (0, I.jsxs)(`div`, {
                    className: `relative w-full min-w-0 sm:w-auto sm:flex-1 md:mx-auto md:max-w-md`,
                    children: [
                      (0, I.jsx)(O, {
                        className: `absolute left-3.5 top-1/2 size-4 -translate-y-1/2 text-muted-foreground`,
                      }),
                      (0, I.jsx)(`input`, {
                        value: S,
                        onChange: (e) =>
                          o ? o(e.target.value) : x(e.target.value),
                        placeholder: `Rechercher une offre, une entreprise…`,
                        className: `h-10 w-full rounded-full border border-border/70 bg-card/60 pl-10 pr-4 sm:pr-14 text-sm outline-none transition-colors placeholder:text-muted-foreground focus:border-primary/50`,
                      }),
                      (0, I.jsx)(`kbd`, {
                        className: `pointer-events-none absolute right-3 top-1/2 hidden -translate-y-1/2 rounded-md border border-border/70 px-1.5 py-0.5 text-[10px] text-muted-foreground sm:block`,
                        children: `⌘K`,
                      }),
                    ],
                  }),
                  (0, I.jsxs)(`button`, {
                    type: `button`,
                    onClick: B,
                    "aria-label": `Notifications`,
                    className: `relative hidden md:grid size-10 shrink-0 place-items-center rounded-full text-muted-foreground transition-colors hover:bg-accent/50 hover:text-foreground`,
                    children: [
                      (0, I.jsx)(h, { className: `size-[18px]` }),
                      (0, I.jsx)(`span`, {
                        className: `absolute right-1.5 top-1.5 grid size-4 place-items-center rounded-full bg-primary text-[9px] font-bold text-primary-foreground`,
                        children: `3`,
                      }),
                    ],
                  }),
                  a &&
                    (0, I.jsxs)(`button`, {
                      type: `button`,
                      onClick: a,
                      className: `press hidden shrink-0 items-center gap-2 rounded-full gradient-hero px-4 py-2.5 text-[13.5px] font-semibold text-primary-foreground shadow-[0_8px_24px_-12px_var(--color-primary)] sm:inline-flex`,
                      children: [
                        (0, I.jsx)(E, { className: `size-4` }),
                        ` Ajouter une opportunité`,
                      ],
                    }),
                  i &&
                    (0, I.jsx)(`div`, {
                      className: `flex shrink-0 flex-wrap items-center gap-2`,
                      children: i,
                    }),
                ],
              }),
            }),
            (0, I.jsxs)(`main`, {
              className: `pop-in mx-auto max-w-[1200px] px-4 pb-28 pt-5 sm:px-6 sm:pt-6 md:pb-12`,
              children: [
                e &&
                  (0, I.jsxs)(`div`, {
                    className: `pop-in mb-5 flex flex-wrap items-end justify-between gap-3 sm:mb-6`,
                    children: [
                      (0, I.jsxs)(`div`, {
                        className: `min-w-0`,
                        children: [
                          n &&
                            (0, I.jsx)(`p`, {
                              className: `mb-1 text-[11px] font-bold uppercase tracking-[0.16em] text-primary`,
                              children: n,
                            }),
                          (0, I.jsx)(`h1`, {
                            className: `text-[22px] font-extrabold tracking-tight sm:text-[28px]`,
                            children: e,
                          }),
                          t &&
                            (0, I.jsx)(`p`, {
                              className: `mt-1 text-sm text-muted-foreground`,
                              children: t,
                            }),
                        ],
                      }),
                      r &&
                        (0, I.jsx)(`div`, {
                          className: `flex w-full flex-wrap items-center gap-2 [&>*]:flex-1 sm:w-auto sm:[&>*]:flex-none`,
                          children: r,
                        }),
                    ],
                  }),
                l,
              ],
            }),
          ],
        }),
        (0, I.jsx)(`nav`, {
          className: `fixed inset-x-0 bottom-0 z-40 border-t border-border/60 bg-background/90 pb-[env(safe-area-inset-bottom)] backdrop-blur-xl md:hidden`,
          children: (0, I.jsxs)(`div`, {
            className: `flex items-end justify-around px-2 pt-1.5`,
            children: [
              (0, I.jsx)(U, {
                to: `/`,
                label: `Accueil`,
                icon: b,
                active: d === `/`,
              }),
              (0, I.jsx)(U, {
                to: `/candidatures`,
                label: `Candidatures`,
                icon: y,
                active: d === `/candidatures`,
              }),
              (0, I.jsx)(`button`, {
                type: `button`,
                onClick: a ?? B,
                "aria-label": `Ajouter une opportunité`,
                className: `press -mt-6 grid size-14 shrink-0 place-items-center self-center rounded-full gradient-hero text-primary-foreground shadow-[0_10px_30px_-10px_var(--color-primary)]`,
                children: (0, I.jsx)(E, { className: `size-6` }),
              }),
              (0, I.jsx)(U, {
                to: `/calendrier`,
                label: `Calendrier`,
                icon: _,
                active: d === `/calendrier`,
              }),
              (0, I.jsx)(U, {
                to: `/profil`,
                label: `Profil`,
                icon: M,
                active: d === `/profil`,
              }),
            ],
          }),
        }),
      ],
    })
  );
}
function U({ to: e, label: t, icon: n, active: r }) {
  let i = f(
      `flex flex-1 flex-col items-center gap-1 py-2 text-[10.5px] font-medium transition-colors`,
      r ? `text-primary` : `text-muted-foreground`,
    ),
    a = (0, I.jsxs)(I.Fragment, {
      children: [(0, I.jsx)(n, { className: `size-[19px]` }), t],
    });
  return e
    ? (0, I.jsx)(p, { to: e, className: i, children: a })
    : (0, I.jsx)(`button`, {
        type: `button`,
        onClick: B,
        className: i,
        children: a,
      });
}
export {
  j as a,
  O as c,
  x as d,
  y as f,
  h as g,
  g as h,
  M as i,
  E as l,
  _ as m,
  P as n,
  A as o,
  v as p,
  N as r,
  k as s,
  H as t,
  w as u,
};
