...
//#region node_modules/.nitro/vite/services/ssr/assets/cv-fichier-WXbTb_ed.js
/**
* Global Polyfills
...
*/
function patchReadableStreamInstance(stream) {
	if (!stream || typeof stream !== "object") return;
	const s = stream;
	const asyncIterSymbol = typeof Symbol !== "undefined" && Symbol.asyncIterator ? Symbol.asyncIterator : Symbol?.asyncIterator || Symbol.for("Symbol.asyncIterator");
	if (typeof s.getReader === "function" && !s[asyncIterSymbol]) s[asyncIterSymbol] = async function* () {
...
function initPolyfills() {
	...
	const gAny = globalScope;
	if (!gAny["process"]) gAny["process"] = { env: {
		NODE_ENV: "development",
		TSS_ROUTER_BASEPATH: ""
	} };
	else {
		const proc = gAny["process"];
	...
	if (!gAny["global"]) gAny["global"] = globalScope;
	if (typeof Symbol !== "undefined" && !Symbol.asyncIterator) Symbol.asyncIterator = Symbol.for("Symbol.asyncIterator");
	const asyncIterSymbol = typeof Symbol !== "undefined" && Symbol.asyncIterator ? Symbol.asyncIterator : Symbol.for("Symbol.asyncIterator");
	const g = globalScope;
	if (g.ReadableStream && g.ReadableStream.prototype) {
		const proto = g.ReadableStream.prototype;
		if (!proto[asyncIterSymbol]) proto[asyncIterSymbol] = async function* () {
		...
		if (!proto["values"]) proto["values"] = proto[asyncIterSymbol];
	}
	if (typeof window !== "undefined") {
		window.addEventListener("vite:preloadError", (event) => {
			event.preventDefault();
			console.warn("[Vite Preload] Handled chunk load error gracefully:", event);
			const key = "chunk_preload_reload";
			const now = Date.now();
			const last = Number(sessionStorage.getItem(key) || "0");
			if (!last || now - last > 15e3) {
				sessionStorage.setItem(key, String(now));
				window.location.reload();
			}
		});
		window.addEventListener("unhandledrejection", (event) => {
			const msg = event.reason instanceof Error ? event.reason.message : typeof event.reason === "string" ? event.reason : "";
		...
		window.addEventListener("error", (event) => {
			...
			if (msg.includes("Importing a module script failed") || msg.includes("Failed to fetch dynamically imported module") || msg.includes("error loading dynamically imported module")) {
				event.preventDefault();
				console.warn("[Module Script Error] Suppressed unhandled script error:", msg);
			}
		});
	}
}
...
async function extrairePdf(file) {
	initPolyfills();
	const pdfjs = await import("../_libs/pdfjs-dist.mjs").then((n) => /* @__PURE__ */ __toESM(n.t()));
	...
	for (let i = 1; i <= doc.numPages; i++) {
		...
		const origStreamTextContent = page.streamTextContent;
		if (typeof origStreamTextContent === "function") page.streamTextContent = function(...args) {
