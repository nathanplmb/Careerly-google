...
//#region node_modules/.nitro/vite/services/ssr/assets/ai-fallbacks-io-4b2vQ.js
var _ai = null;
...
function nettoyerLigneBruitWeb(ligne) {
	...
	if (/^(offres|offres d'emploi|événements|entreprises|ressources|conseils|découvrir les métiers|recherche|filtres|rechercher une offre|accueil >)/i.test(l)) return true;
	if (/^(langue sélectionnée|français|english|español|deutsch|italiano|português)/i.test(l)) return true;
	if (/^(connexion|se connecter|créer un compte|s'inscrire|mon compte|mon profil|mes candidatures|mes alertes|favoris|sauvegarder|enregistrer|partager|postuler|postuler maintenant|postuler sur le site|candidater)/i.test(l)) return true;
	...
	if (/^(partager sur|partager par|imprimer l'offre|signaler cette offre|voir l'offre originale|offres similaires|voir toutes les offres)/i.test(l)) return true;
	if (/^(\d+(\.\d+)?\s*\/\s*5|\d+\s*avis|note globale)/i.test(l)) return true;
	return false;
}
function fallbackExtraireOffre(texte) {
	const lignesUtiles = texte.split("\n").map((l) => l.trim()).filter(Boolean).filter((l) => !nettoyerLigneBruitWeb(l));
	...
	for (const v of [
		...
		"Télétravail",
		"Remote"
	]) if (new RegExp(`\\b${v.replace("(", "\\(").replace(")", "\\)")}\\b`, "i").test(texteNettoye)) {
		lieu = v;
		break;
	}
	if (!lieu) {
	...
	for (let i = 0; i < Math.min(lignesUtiles.length, 12); i++) {
		...
		if (!poste && /(?:Stage|Alternance|CDI|CDD|Apprentissage|Consultant|Chef de projet|Développeur|Ingénieur|Assistant|Analyste|Manager|Bras droit|Business Developer|Product|Data|Designer|Juriste|Chargé|Stagiaire|Internship|Trainee)/i.test(l)) {
			const candidate = l.replace(/^(Poste|Intitulé|Titre)\s*:\s*/i, "").trim();
			if (candidate.includes(" | ")) {
				const parts = candidate.split(" | ").map((p) => p.trim());
				poste = parts[0];
				for (let j = 1; j < parts.length; j++) if (!isNomSiteRecrutement(parts[j]) && !entreprise) entreprise = parts[j];
			} else if (candidate.includes(" - ")) {
				const parts = candidate.split(" - ").map((p) => p.trim());
				poste = parts[0];
				if (parts.length >= 2 && !/^(F\/H|H\/F|F\/M|w\/m\/d|Stage|CDI|CDD|Paris|France)$/i.test(parts[1]) && !isNomSiteRecrutement(parts[1]) && !entreprise) entreprise = parts[1];
			} else poste = candidate;
			poste = poste.replace(/\s*\|\s*(jobteaser|welcome to the jungle|linkedin|indeed|hellowork).*$/i, "").trim();
		}
	}
	if (!entreprise) {
		const matchIntro = texteNettoye.match(/(?:À propos de|À propos d'|About|Rejoindre|Rejoignez|Chez|Le groupe|La société|L'entreprise)\s+([A-ZÀ-Ý][A-Za-z0-9À-ÿ&'.\s-]{2,35})/i);
		if (matchIntro && !isNomSiteRecrutement(matchIntro[1])) entreprise = matchIntro[1].replace(/[:,\n].*$/, "").trim();
	}
	...
	if (!entreprise && matchEmail) {
		...
		if (domaine && !/^(gmail|hotmail|yahoo|outlook|live|icloud|jobteaser|wanadoo|orange|free|sfr)$/i.test(domaine)) entreprise = domaine.charAt(0).toUpperCase() + domaine.slice(1);
	}
	const corpsRecherche = `${poste} ${entreprise} ${texteNettoye}`.toLowerCase();
	if (/ia|intelligence artificielle|genai|machine learning|deep learning|data scientist|data analyst|data engineer|python|power bi|software|cloud|cybers[ée]curit[ée]|d[ée]veloppeur|devops|fullstack|saas/i.test(corpsRecherche)) secteur = "Tech & IA";
	else if (/conseil|consulting|transformation digitale|strat[ée]gie|conduite du changement|esn|wavestone|capgemini|sopra steria|bcg|mckinsey|bain|accenture/i.test(corpsRecherche)) secteur = "Conseil & Stratégie";
	else if (/m&a|fusion|acquisition|private equity|asset management|banque|finance de march[ée]|trading|risques|tr[ée]sorerie|analyste financier|bnp|soci[ée]t[ée] g[ée]n[ée]rale|cr[ée]dit agricole|rothschild|lazard/i.test(corpsRecherche)) secteur = "Finance & Banque";
	else if (/luxe|cosm[ée]tique|parfum|mode|haute couture|maroquinerie|joaillerie|beaut[ée]|lvmh|kering|l'or[ée]al|chanel|herm[èe]s|dior|gucci|richemont/i.test(corpsRecherche)) secteur = "Luxe & Cosmétiques";
	else if (/audit|expertise comptable|contr[ôo]le de gestion|commissariat|comptabilit[ée]|pwc|ey|deloitte|kpmg|mazars|bdo/i.test(corpsRecherche)) secteur = "Audit & Contrôle de gestion";
	else if (/marketing|communication|brand|social media|growth|publicit[ée]|m[ée]dias|content|relations presse|influence|[ée]v[ée]nementiel/i.test(corpsRecherche)) secteur = "Marketing & Communication";
	else if (/sant[ée]|pharmaceutique|pharma|biotech|m[ée]dical|dispositifs m[ée]dicaux|sanofi|pfizer|novartis/i.test(corpsRecherche)) secteur = "Santé & Pharma";
	else if (/industrie|automobile|a[ée]ronautique|[ée]nergie|totalenergies|schneider|airbus|safran|renault|stellantis|supply chain|logistique/i.test(corpsRecherche)) secteur = "Industrie & Énergie";
	else if (/e-commerce|retail|grande distribution|amazon|fnac|d[ée]cathlon|carrefour|auchan|merchandising/i.test(corpsRecherche)) secteur = "E-commerce & Retail";
	else if (/ressources humaines|rh|recrutement|talent acquisition|campus management|people & culture|sirh|formation/i.test(corpsRecherche)) secteur = "RH & Recrutement";
	else if (/droit|juridique|avocat|droit des affaires|juriste|conformit[ée]|compliance/i.test(corpsRecherche)) secteur = "Droit & Juridique";
	else if (/agroalimentaire|danone|nestl[ée]|lactalis/i.test(corpsRecherche)) secteur = "Agroalimentaire";
	else if (/immobilier|btp|bouygues|vinci|eiffage/i.test(corpsRecherche)) secteur = "Immobilier & BTP";
	else secteur = "Conseil & Services";
	...
	for (const ligne of lignesUtiles) {
		const l = ligne.trim();
		if (!l) continue;
		if (poste && l.includes(poste) && /jobteaser|welcome to the jungle|linkedin|indeed/i.test(l)) continue;
		if (/(?:missions|responsabilités|vos missions|ce que vous ferez|le poste|tâches|votre rôle|activités principales)/i.test(l) && l.length < 65) {
			sectionActuelle = "missions";
			continue;
		}
		if (/(?:profil|profil recherché|votre profil|compétences|prérequis|qui êtes-vous|votre profil idéal|qualités requises|ce que nous recherchons)/i.test(l) && l.length < 65) {
			sectionActuelle = "profil";
			continue;
		}
		if (/(?:conditions|modalités|avantages|rémunération|gratification|rythme|durée du contrat|type de contrat|début)/i.test(l) && l.length < 65) {
			sectionActuelle = "conditions";
			continue;
		}
		if (/(?:à propos|l'entreprise|qui sommes-nous|notre groupe|contexte)/i.test(l) && l.length < 65) {
	...
	if (sections.missions.length > 0) {
		...
		const mNettoyees = sections.missions.filter((m) => m.length > 10 && m.length < 200).slice(0, 4);
	...
	if (sections.profil.length > 0) {
		...
		const pNettoyees = sections.profil.filter((p) => p.length > 10 && p.length < 200).slice(0, 3);
	...
	if (sections.conditions.length > 0) {
		...
		const cNettoyees = sections.conditions.filter((c) => c.length > 5 && c.length < 150).slice(0, 2);
	...
	if (!resume || resume.length < 40) resume = `🎯 **Points clés de l'offre :**\n${sections.autres.filter((l) => l.length > 15 && l.length < 250).slice(0, 4).map((l) => `• ${l.replace(/^[-•*]\s*/, "")}`).join("\n")}`;
	let commentaire = "";
	const entNom = entreprise || "l'entreprise";
	const postNom = poste || "cette offre";
	if (/tech|ia|data/i.test(secteur)) commentaire = `${postNom} chez ${entNom}. Valorisez vos projets pratiques (IA, Python, Data) et votre capacité d'adaptation.`;
	else if (/conseil|stratégie/i.test(secteur)) commentaire = `${postNom} chez ${entNom}. Mettez en avant votre esprit de synthèse, posture client et compétences analytiques.`;
	else if (/finance|banque|audit/i.test(secteur)) commentaire = `${postNom} chez ${entNom}. Soignez la rigueur, la maîtrise d'outils financiers et la solidité de votre parcours.`;
	else if (/luxe/i.test(secteur)) commentaire = `${postNom} chez ${entNom}. Soignez l'excellence du détail, l'attrait pour l'univers de marque et la sensibilité produit.`;
	else if (/marketing|communication/i.test(secteur)) commentaire = `${postNom} chez ${entNom}. Mettez en valeur vos réalisations chiffrées, créativité et sens du storytelling.`;
	else if (poste && entreprise) commentaire = `Offre de ${poste} chez ${entreprise}. Relance proactive conseillée à J+8.`;
	else commentaire = "Offre importée et analysée. Complétez vos démarches et soignez votre accroche.";
	return {
