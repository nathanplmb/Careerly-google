...
//#region node_modules/.nitro/vite/services/ssr/assets/brief.functions-DClaqNgb.js
var Input = object({
	faits: string().min(1),
...
var genererBrief = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data) => Input.parse(data)).handler(genererBrief_createServerFn_handler, async ({ data, context }) => {
	...
	const { genererBriefIA, MODELE_BRIEF } = await import("./brief.server-D-LHYiJp.mjs");
	return {
