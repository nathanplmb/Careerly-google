...
//#region node_modules/.nitro/vite/services/ssr/assets/matching.functions-CzEjfPmO.js
var Input = object({
	profil: string().min(1),
	offre: string().min(10)
});
...
var analyserCorrespondance = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data) => Input.parse(data)).handler(analyserCorrespondance_createServerFn_handler, async ({ data, context }) => {
	...
	const { analyserCorrespondanceIA, MODELE_MATCH } = await import("./matching.server-BPu71kxh.mjs");
	return {
