//#region node_modules/.nitro/vite/services/ssr/assets/chat.prompt-kSSU_L-J.js
var PERSONA_PROMPTS = {
	general_advisor: {
...
function buildFullSystemInstruction(payload) {
	const base = payload.personaId === "custom" && payload.customSystemInstruction?.trim() ? payload.customSystemInstruction.trim() : PERSONA_PROMPTS[payload.personaId]?.systemPrompt || PERSONA_PROMPTS.general_advisor.systemPrompt;
	let contextAddon = "";
	if (payload.candidateContext) {
		const parts = [];
		if (payload.candidateContext.name) parts.push(`Nom/Prénom du candidat : ${payload.candidateContext.name}`);
		if (payload.candidateContext.titreVise) parts.push(`Poste ou rôle visé : ${payload.candidateContext.titreVise}`);
		if (payload.candidateContext.secteur) parts.push(`Secteur d'activité : ${payload.candidateContext.secteur}`);
		if (payload.candidateContext.formation) parts.push(`Formation : ${payload.candidateContext.formation}`);
		if (payload.candidateContext.experiences) parts.push(`Expériences : ${payload.candidateContext.experiences}`);
		if (payload.candidateContext.competences?.length) parts.push(`Compétences clés : ${payload.candidateContext.competences.join(", ")}`);
		if (payload.candidateContext.langues?.length) parts.push(`Langues : ${payload.candidateContext.langues.join(", ")}`);
		if (payload.candidateContext.localisation) parts.push(`Localisation / Mobilité : ${payload.candidateContext.localisation}`);
		if (payload.candidateContext.contrats) parts.push(`Type de contrat recherché : ${payload.candidateContext.contrats}`);
		if (payload.candidateContext.remuneration) parts.push(`Prétentions salariales : ${payload.candidateContext.remuneration}`);
		if (payload.candidateContext.objectifs) parts.push(`Objectifs de recherche : ${payload.candidateContext.objectifs}`);
		if (parts.length > 0) contextAddon = `\n\n### Contexte du profil candidat :\n${parts.join("\n")}`;
	}
...
//#endregion
export { buildFullSystemInstruction as n, PERSONA_PROMPTS as t };
