...
//#region node_modules/.nitro/vite/services/ssr/assets/redaction.functions-I4l6Huza.js
var Entree = object({
...
var genererLettre = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((data) => Entree.parse(data)).handler(genererLettre_createServerFn_handler, async ({ data, context }) => {
	const { consommerQuota, limiterEntree } = await import("./quota.server-DCy6e-0g.mjs");
	await consommerQuota(context.supabase, "redaction");
	const { genererLettreIA } = await import("./redaction.server-DlVFyxmY.mjs");
...
var genererLinkedin = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((data) => Entree.parse(data)).handler(genererLinkedin_createServerFn_handler, async ({ data, context }) => {
	const { consommerQuota, limiterEntree } = await import("./quota.server-DCy6e-0g.mjs");
	await consommerQuota(context.supabase, "redaction");
	const { genererLinkedinIA } = await import("./redaction.server-DlVFyxmY.mjs");
...
var genererInterview = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).validator((data) => Entree.parse(data)).handler(genererInterview_createServerFn_handler, async ({ data, context }) => {
	const { consommerQuota, limiterEntree } = await import("./quota.server-DCy6e-0g.mjs");
	await consommerQuota(context.supabase, "redaction");
	const { genererInterviewIA } = await import("./redaction.server-DlVFyxmY.mjs");
