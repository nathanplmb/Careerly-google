...
import { t as createServerRpc } from "./createServerRpc-neQCt8ex.mjs";
//#region node_modules/.nitro/vite/services/ssr/assets/profil-ia.functions-CIeH79ot.js
...
var genererSyntheseProfil = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data) => SyntheseInput.parse(data)).handler(genererSyntheseProfil_createServerFn_handler, async ({ data, context }) => {
	...
	const { genererSyntheseProfilIAServer } = await import("./profil-ia.server-BBCXmQyV.mjs");
...
var optimiserProfilIA = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth]).inputValidator((data) => OptimiserInput.parse(data)).handler(optimiserProfilIA_createServerFn_handler, async ({ data, context }) => {
	const { consommerQuota } = await import("./quota.server-DCy6e-0g.mjs");
	await consommerQuota(context.supabase, "profil_audit").catch(() => void 0);
	const { optimiserProfilIAServer } = await import("./profil-ia.server-BBCXmQyV.mjs");
