import {
  A as isMobileCordova,
  C as getModularInstance,
  D as isCloudflareWorker,
  E as isCloudWorkstation,
  F as querystringDecode,
  N as pingServer,
  O as isEmpty,
  P as querystring,
  S as getExperimentalSetting,
  T as isBrowserExtension,
  _ as createSubscribe,
  a as getApp,
  b as getDefaultEmulatorHost,
  c as registerVersion,
  d as Component,
  f as Deferred,
  h as base64Decode,
  i as _registerComponent,
  j as isReactNative,
  k as isIE,
  l as LogLevel,
  m as FirebaseError,
  n as _getProvider,
  p as ErrorFactory,
  r as _isFirebaseServerApp,
  t as SDK_VERSION,
  u as Logger,
  v as deepEqual,
  w as getUA,
  y as extractQuerystring,
} from "./@firebase/app+[...].mjs";
...
async function _performFetchWithErrorHandling(auth, customErrorMap, fetchFn) {
  ...
  try {
    ...
    if (response.ok && !("errorMessage" in json)) return json;
      ...
      const [serverErrorCode, serverErrorMessage] = (
        response.ok ? json.errorMessage : json.error.message
      ).split(" : ");
...
async function _reloadWithoutSaving(user) {
  ...
  const response = await _logoutIfInvalidated(
    user,
    getAccountInfo(auth, { idToken: await user.getIdToken() }),
  );
...
var AuthImpl = class {
  ...
  async _updatePasswordPolicy() {
    const passwordPolicy = new PasswordPolicyImpl(
      await _getPasswordPolicy(this),
    );
...
var RecaptchaEnterpriseVerifier = class RecaptchaEnterpriseVerifier {
  ...
  async verify(action = "verify", forceRefresh = false) {
    ...
    if (this.auth.settings.appVerificationDisabledForTesting)
      return new MockGreCAPTCHATopLevel().execute("siteKey", {
        action: "verify",
      });
    return new Promise((resolve, reject) => {
...
async function handleRecaptchaFlow(
  ...
  if (recaptchaAuthProvider === "EMAIL_PASSWORD_PROVIDER") {
    if (
      authInstance
        ._getRecaptchaConfig()
        ?.isProviderEnabled("EMAIL_PASSWORD_PROVIDER")
    )
      return actionMethod(
        authInstance,
        await injectRecaptchaFields(
          authInstance,
          request,
          actionName,
    ...
    if (authInstance._getRecaptchaConfig()?.isProviderEnabled("PHONE_PROVIDER"))
      return actionMethod(
        authInstance,
        await injectRecaptchaFields(authInstance, request, actionName),
      ).catch(async (error) => {
        if (
          authInstance
            ._getRecaptchaConfig()
...
async function _initializeRecaptchaConfig(auth) {
  ...
  const config = new RecaptchaConfig(
    await getRecaptchaConfig(authInternal, {
      clientType: "CLIENT_TYPE_WEB",
      version: "RECAPTCHA_ENTERPRISE",
    }),
  );
  ...
  if (config.isAnyProviderEnabled())
    new RecaptchaEnterpriseVerifier(authInternal).verify();
}
/**
 * @license
 * Copyright 2020 Google LLC
...
var OAuthCredential = class OAuthCredential extends AuthCredential {
   ...
   */
  static fromJSON(json) {
    const { providerId, signInMethod, ...rest } =
  ...
  _addListener(key, listener) {
    ...
    if (window.cookieStore) {
      const cb = (event) => {
        ...
        if (event.deleted.find((change) => change.name === name))
          listener(null);
      };
      const unsubscribe = () =>
...
var Receiver = class Receiver {
  ...
  async handleEvent(event) {
    ...
    const response = await _allSettled(
...
async function getObject(db, key) {
  const data = await new DBPromise(
    getObjectStore(db, false).get(key),
  ).toPromise();
...
var IndexedDBLocalPersistence = class {
  ...
  notifyListeners(key, newValue) {
    ...
    const listeners = this.listeners[key];
    if (listeners)
      for (const listener of Array.from(listeners)) listener(newValue);
  }
  startPolling() {
    this.stopPolling();
    this.pollTimer = setInterval(
...
async function _verifyPhoneNumber(auth, options, verifier) {
  ...
  try {
    ...
    if (typeof options === "string")
      phoneInfoOptions = { phoneNumber: options };
      ...
      if ("phoneNumber" in phoneInfoOptions) {
        ...
        const startEnrollPhoneMfaActionCallback = async (
          ...
          if (request.phoneEnrollmentInfo.captchaResponse === FAKE_TOKEN) {
            ...
            return startEnrollPhoneMfa(
              authInstance,
              await injectRecaptchaV2Token(authInstance, request, verifier),
            );
          }
...
async function signInWithPopup(auth, provider, resolver) {
  if (_isFirebaseServerApp(auth.app))
    return Promise.reject(
      _createError(auth, "operation-not-supported-in-this-environment"),
    );
  const authInternal = _castAuth(auth);
  _assertInstanceOf(auth, provider, FederatedAuthProvider);
  return new PopupOperation(
    authInternal,
    "signInViaPopup",
    provider,
    _withDefaultResolver(authInternal, resolver),
  ).executeNotNull();
}
/**
 * Popup event manager. Handles the popup's entire lifecycle; listens to auth
...
var RedirectAction = class extends AbstractPopupRedirectOperation {
  ...
  async execute() {
    ...
    if (!readyOutcome) {
      try {
        const result = (await _getAndClearPendingRedirectStatus(
          this.resolver,
          this.auth,
        ))
          ? await super.execute()
...
async function _getRedirectResult(
  ...
  const authInternal = _castAuth(auth);
  const result = await new RedirectAction(
    authInternal,
    _withDefaultResolver(authInternal, resolverExtern),
    bypassAuthState,
  ).execute();
...
function resetUnloadedGapiModules() {
  ...
  if (beacon?.H)
    for (const hint of Object.keys(beacon.H)) {
      ...
      if (beacon.CP)
        for (let i = 0; i < beacon.CP.length; i++) beacon.CP[i] = null;
    }
}
...
async function _getRedirectUrl(
  ...
  for (const key of Object.keys(paramsDict))
    if (paramsDict[key] === void 0) delete paramsDict[key];
  ...
  async initAndGetManager(auth) {
    ...
    iframe.register(
      ...
      (iframeEvent) => {
        ...
        return {
          status: manager.onEvent(iframeEvent.authEvent) ? "ACK" : "ERROR",
        };
      },
...
var AuthInterop = class {
  ...
  async getToken(forceRefresh) {
    ...
    return {
      accessToken: await this.auth.currentUser.getIdToken(forceRefresh),
    };
  }
    ...
    new Component(
      ...
      (container) => {
        return ((auth) => new AuthInterop(auth))(
          _castAuth(container.getProvider("auth").getImmediate()),
        );
      },
      "PRIVATE",
    ).setInstantiationMode("EXPLICIT"),
