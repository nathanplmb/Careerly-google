...
const STYLES: Record<
  Statut,
  { bg: string; text: string; dot: string; glow?: string }
> = {
  Sauvegardée: {
...
export function StatutBadge({ statut, size = "md" }: Props) {
  ...
  return (
    <span
      className={`inline-flex items-center gap-1.5 whitespace-nowrap rounded-full font-medium select-none shadow-[inset_0_1px_0_rgba(255,255,255,0.12)] transition-all ${sizeClass} ${cfg.bg} ${cfg.text}`}
    >
