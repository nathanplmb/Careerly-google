...
function getFirebaseAdminApp() {
	...
	const projectId = process.env.FIREBASE_PROJECT_ID || firebase_applet_config_default.projectId || "gen-lang-client-0123496230";
...
async function verifyIsAdmin(idToken) {
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur vérification collection admins:", err);
	}
	return {
		isAdmin: false,
		isSuperAdmin: false,
...
async function listAllUsersAdmin() {
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur chargement liste admins:", err);
	}
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Échec récupération admin.auth().listUsers (fallback Firestore actif):", authErr);
	}
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur chargement profils Firestore:", profErr);
	}
...
async function searchAndDiagnoseUser(queryStr) {
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur lecture rôle admin:", e);
	}
	let profData = null;
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur lecture profil Firestore:", e);
	}
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur comptage collections:", e);
	}
	let diag = "standard";
...
async function executeCascadeAccountDeletion({ targetUid, requesterUid, isSelfDeletion }) {
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur suppression sous-collections Firestore:", err);
	}
	try {
		await db.collection("profils").doc(targetUid).delete();
		details.profileDeleted = true;
	} catch (err) {
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur suppression profil:", err);
	}
	try {
		await db.collection("admins").doc(targetUid).delete();
	} catch (e) {
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur suppression document admin:", e);
	}
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur suppression fichiers Storage:", err);
	}
	try {
		await auth.deleteUser(targetUid);
		details.authDeleted = true;
	} catch (err) {
		if (!(err instanceof Error ? err.message : String(err)).includes("user-not-found") && process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur suppression Firebase Auth:", err);
	...
	try {
		...
		if (process.env.FIREBASE_SERVICE_ACCOUNT_KEY) console.warn("Erreur écriture audit_logs:", err);
	}
	return {
		success: true,
